<?php
/**
 * Template for PDF license keys table.
 *
 * Version: 1.0
...
if (get_option('fslm_download_links', 'on') == 'on') {
	$download = '<div class="fslm-download-keys" style="margin-bottom: 10px">
                    <a class="button" href="' . get_site_url() . '?fslm-order-id=' . $order_id . '">' . __('Download as a CSV file',
			'fslm') . '</a>
                    <a class="button" href="' . get_site_url() . '?fslm-order-id-txt=' . $order_id . '">' . __('Download as a TXT file',
			'fslm') . '</a>
                </div>';
}

$val = '<h4>' . (count($values) == 1 ? get_option('fslm_meta_key_name',
		'License Key') : get_option('fslm_meta_key_name_plural', 'License Key')) . '</h4>' . $download .
       '<table class="td fslm-license-keys-table" style="border-collapse: collapse; width: 100%; border: 1px solid #e5e5e5; font-family: \'Helvetica Neue\', Helvetica, Roboto, Arial, sans-serif;">' .
       '<thead>' .
       '<tr>' .
